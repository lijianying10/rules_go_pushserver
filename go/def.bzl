...
go_library_attrs = dicts.add(go_env_attrs, {
    "data": attr.label_list(
        allow_files = True,
    ),
    "srcs": attr.label_list(allow_files = _GO_FILETYPES),
...
go_link_attrs = dicts.add(go_library_attrs, _crosstool_attrs, {
...
go_library = rule(
    go_library_impl,
    attrs = dicts.add(go_library_attrs, {
...
go_binary = rule(
    go_binary_impl,
    attrs = dicts.add(go_library_attrs, _crosstool_attrs, go_link_attrs, {
        "linux_disable_static_linking": attr.bool(doc = "DEPRECATED!!!"),
    }),
    executable = True,
    fragments = ["cpp"],
)
...
_cgo_codegen_rule = rule(
    _cgo_codegen_impl,
    attrs = dicts.add(go_env_attrs, _crosstool_attrs, {
        "srcs": attr.label_list(
            allow_files = _GO_FILETYPES,
            non_empty = True,
        ),
        "c_hdrs": attr.label_list(
            allow_files = _CPP_HEADERS,
        ),
        "deps": attr.label_list(
            allow_files = False,
            providers = ["cc"],
...
_cgo_object = rule(
    _cgo_object_impl,
    attrs = dicts.add(_crosstool_attrs, {
        "src": attr.label(
            mandatory = True,
            providers = ["cc"],
        ),
...
def _setup_cgo_library(name, srcs, cdeps, copts, clinkopts, go_tool, toolchain):
    ...
    c_hdrs = [s for s in srcs if any([s.endswith(ext) for ext in _CPP_HEADERS])]
